...
if(process.platform === 'darwin') {
  ...
  module.exports.set = rawValue => {
    ...
    return input(rawValue)
      ...
      .then(rawMaxBrightness => {
        ...
        const brightness = Math.floor(parsedValue / 100 * maxBrightness).toString();  // 0〜1 で指定されるようにする
  ...
  module.exports.get = () => execFile('wmic', ['/NAMESPACE:\\\\root\\WMI', '/NODE:localhost', 'path', 'WmiMonitorBrightness', 'get', 'CurrentBrightness', '/FORMAT:VALUE'])
    .then(result => {
      const brightnessStr = result.stdout.replace((/\s/g), '').replace((/CurrentBrightness=(.*)/u), '$1');
      if(brightnessStr === '') return Promise.reject(new Error('Unable to find any monitors to read brightness levels from'));
      return Number(brightnessStr);  // 0～100 で返る
    });
  module.exports.set = rawValue => input(rawValue)
    .then(value => execFile(path.join(__dirname, 'brightness-win32.exe'), ['setbrightness', value]))  // nircmd.exe
    .then(_result => {});  // `undefined` を返すようにしておく;
}
else {
