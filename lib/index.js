...
if(process.platform === 'darwin') {
  ...
  const getBrightness = stdout => {
    ...
    return Promise.resolve((Number(brightness.value) / Number(brightness.max) * 100).toFixed(0));  // 0〜100 で出力する
  };
  ...
  module.exports.get = () => getBacklight()
    ...
    .then(results => (Number(results[1]) / Number(results[0]) * 100).toFixed(0));  // 0〜100 で出力する
  module.exports.set = rawValue => {
    ...
    return input(rawValue)
      ...
      .then(rawMaxBrightness => {
        ...
        return setBrightness(detectedDevice, brightness);
      })
      .catch(error => {
